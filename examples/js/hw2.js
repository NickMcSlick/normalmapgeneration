// HW2 CS6410 Bryce Paubel 3/5/2022
// This an object viewer program which allows the user to select multiple
// texture/normal pairs, both of which can be turned on or off

// This is based off the normal object viewer shown in class
// Any edits are marked with /***** MAJOR EDIT *****/
...
var frag_phong = `#version 300 es
    ...
    void main() {
        ...
        /***** MAJOR EDIT *****/
        // If the bump map is not defined, don't translate the bump vector
        // The normal will stay the interpolated normal for Phong shading
        if (u_is_bump) {
...
function main () {
    ...
    /***** MAJOR EDITS *****/
    // Modified the size of the canvas
    ...
    /***** MAJOR EDITS *****/
    // This resizes the canvas element when the window changes
    ...
    //get_obj_file(url, 0.05, true); // utah-teapot.obj
    get_obj_file(url, 0.3, true); // sphere.obj
    //get_obj_file(url, 1, true);
    //get_obj_file(url, 2, true); // create2.obj 
    //get_obj_file(url, 3, true);
    //get_obj_file(url, 5, true);
    //get_obj_file(url, 10, true);
    //get_obj_file(url, 30, true);
    //get_obj_file(url, 70, true); // heart.obj 
    //get_obj_file(url, 500, true);    
...
function draw_obj (program, vp_mat) {
    ...
    if (g_texture.length > 0) {
        ...
        /***** MAJOR EDIT *****/
        // If the user says no diffuse map, then tell the shader to not use one
        if (config.TEXTURE_MAP) {
            gl.uniform1i(program.uniforms.u_image, textureChoice);
            gl.uniform1i(program.uniforms.u_is_texture, true);
        } else {
            gl.uniform1i(program.uniforms.u_image,  textureChoice);
            gl.uniform1i(program.uniforms.u_is_texture, false);
        }
    }
    
    if (g_texture.length > 0) { // normal map exists
        // Pass the texure unit to u_image
        /***** MAJOR EDIT *****/
        // If the user says no normal map, then tell the shader to not use one
        if (config.NORMAL_MAP) {
            gl.uniform1i(program.uniforms.u_bump, normalChoice);
            gl.uniform1i(program.uniforms.u_is_bump, true);
        } else {
            gl.uniform1i(program.uniforms.u_bump, normalChoice);
            gl.uniform1i(program.uniforms.u_is_bump, false);
        }
    }   
...
MTL.prototype.parse_mtl_file = function(file_string) {
    ...
    while ( (line = lines[index++]) != null ) {
        ...
        case 'map_Kd': // Read .png file for diffuse material color         
            var url = this.parse_map_Kd(sp); // return image file name
            // url = "http://www.cs.umsl.edu/~kang/htdocs/models/image.png"

            /***** MAJOR EDITS *****/
            // Get texture data from texture pair array 
            for (let i = 0; i < texPair.length; i++) {
                url = "../images/" + texPair[i].diffuse;
                console.log(url);
        ...
        case 'map_Bump': // Read .png file for normal map        
            var url = this.parse_map_Bump(sp); // return image file name
            // url = "http://www.cs.umsl.edu/~kang/htdocs/models/image.png"
